STORY KEY: e04s07
TITLE:     Document and verify conversations offline
TYPE:      Story
PARENT:    e04
STATUS:    Done
...
Streaming/session behavior must be documented with security posture and proven offline.

### 2. Value statement [reviewed]

README and check.sh make P4 behavior and limits clear without live providers.
...
```gherkin
Feature: P4 docs and gate
  Scenario: README documents sessions and streaming
    When a user reads README
    Then session path, permissions, retention, and stream opt-in are described
    And encryption non-goal is explicit

  Scenario: Mechanical gate green
    When ./scripts/check.sh runs
    Then all offline tests and package smoke pass
```
