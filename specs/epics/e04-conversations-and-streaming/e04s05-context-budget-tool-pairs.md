STORY KEY: e04s05
TITLE:     Budget context without splitting tool pairs
TYPE:      Story
PARENT:    e04
STATUS:    Done
...
Long sessions will exceed context. Truncation must never leave a tool result without its call (or vice versa).

### 2. Value statement [reviewed]

History sent to the model preserves complete tool-call/result pairs under a budget.
...
Feature: Context budget
  Scenario: Drops oldest complete pairs first
    Given a history exceeding the budget with multiple tool pairs
    When budgeting is applied
    Then no tool result remains without its assistant tool_call
    And the newest pairs are preferred for retention

  Scenario: Single oversized pair handling is explicit
    Given one tool pair larger than the budget
    When budgeting is applied
    Then OrbitRelay fails closed or keeps only allowed system/user prefix per documented policy
```
