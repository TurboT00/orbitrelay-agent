STORY KEY: e04s01
TITLE:     Emit a correlated run event model
TYPE:      Story
PARENT:    e04
STATUS:    Todo
...
Runs currently mix final print, verbose usage lines, and approval audit strings without a single machine-readable timeline. P4 needs one event model before streaming or persistence.

### 2. Value statement [reviewed]

Every agent run produces ordered, correlated events for model, tools, approvals, usage, errors, and completion.
...
```gherkin
Feature: Run event model
  Scenario: Tool round emits correlated events
    Given a fake model that requests write_file then returns final text
    When the agent runs with an event collector
    Then events include tool.requested, approval.decided, tool.result, and run.completed
    And tool events share the same tool_call_id

  Scenario: Events are secret-free
    Given a tool result containing an api_key-looking value
    When events are serialized
    Then credential-like fields are redacted
```
