...
**Development status:** post-0.5.0 stabilization at identity 0.6.0; e10 complete; release candidate accepted; publish approval required
...
epics through e10 are complete with 72 passing tasks; the revision-bound
disposition still covers all 26 July findings with no open blockers; the
standalone release candidate is accepted. Automated verification is the release
gate; user-run side testing remains optional and non-blocking. Tagging, pushing,
and publishing still require separate release-owner approval.
...
3.12-3.14 matrix with truthful platform claims. The project is paused only for
explicit next-story approval (recommended: none).
...
| 0.6.0 | Trustworthy Stabilization (candidate accepted) | Candidate ready |
